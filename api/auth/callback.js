...
function isValidCallbackUri(uri) {
  try {
    ...
    // اسمح فقط بالـ callback paths اللي انت مسجلهم فعلاً في Google Console
...
function pickRedirectUri(req) {
  ...
  // Dynamic (safe) on current origin
  const dynamic = `${origin}/api/auth/callback`;

  // لو مفيش candidates خالص: استخدم dynamic
  if (candidates.length === 0) {
    // في local ممكن تكون مسجل /oauth2callback فقط
    return isLocal ? `${origin}/oauth2callback` : dynamic;
  }

  // اختار candidate بنفس origin الحالي فقط (أمان + يمنع مشاكل Vercel)
  ...
  // Local fallback لو Google Console عنده localhost/oauth2callback
  if (isLocal) return `${origin}/oauth2callback`;

  // مهم: ما ترجعش URI على origin مختلف (ده ممكن يعمل Vercel DEPLOYMENT_NOT_FOUND)
  // فالأفضل نرجع dynamic على نفس الـ origin الحالي
  return dynamic;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (newRefreshToken) {
      ...
      res.send(
        "<h1>⚠️ Warning</h1><p>Auth successful but no refresh token returned. Try removing the app permissions from your Google account and trying again.</p>"
      );
    }
  } catch (error) {
