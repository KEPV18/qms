...
const apiBase = (import.meta as unknown as { env?: Record<string, unknown> }).env?.DEV
  ? "http://localhost:3001"
  : "";
...
// (موجودين كـ helpers للاستخدام المستقبلي)
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _activatedEmailsHelpers = { loadActivatedEmails, saveActivatedEmails };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const bootstrap = async () => {
      if (supabase && !supabaseDisabled) {
        ...
        if (error) {
          // keep Supabase enabled; errors will be surfaced via UI flows if needed
          void 0;
        }
  ...
  const login = React.useCallback(
    (email: string, password: string) => {
      ...
      setUsers(updated);
      if (!supabase || supabaseDisabled) saveUsersLocal(updated);
  ...
  const addUser = React.useCallback(
    async (userInput: Omit<AppUser, "id">) => {
      ...
      if (supabase && !supabaseDisabled) {
        ...
        // Supabase failed -> fallback to backend API then local
      ...
      // No Supabase -> try backend API then local
      try {
        const created = await apiFetch<AppUser>("/api/users", {
          method: "POST",
          body: JSON.stringify(newUser),
  ...
  const reloadUsers = React.useCallback(async () => {
    ...
    // Not Supabase -> try backend API first, then local
    try {
      const existing = await apiFetch<AppUser[]>("/api/users");
      setUsers(existing);
      ...
      const currentId = loadSession();
      if (currentId) {
        const u = existing.find((x) => x.id === currentId) || null;
        setUser(u);
      }
    } catch {
      ...
      const currentId = loadSession();
      if (currentId) {
        const u = localUsers.find((x) => x.id === currentId) || null;
        setUser(u);
      }
    }
  }, [supabaseDisabled]);
