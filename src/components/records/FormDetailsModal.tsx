...
export function FormDetailsModal({ record, open, onOpenChange, onViewInDrive }: FormDetailsModalProps) {
  ...
  const getStatusColor = () => {
    ...
    if (record.daysUntilNextFill && record.daysUntilNextFill <= 7) return "warning";
    return "success";
  };
  ...
  const getDepartment = () => {
    // Extract department from category or code
    ...
    const codePrefix = record.code.split('/')[0];
    return deptMap[codePrefix] || "General";
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
          ...
          {record.reviewed && (
            <div className="bg-success/10 rounded-lg p-4">
              ...
              <div className="text-sm text-success">
                Reviewed by {record.reviewedBy} on {record.reviewDate}
              </div>
            </div>
          )}
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          ...
          <Button onClick={onViewInDrive} className="gap-2">
